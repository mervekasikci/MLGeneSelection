...
source("https://raw.githubusercontent.com/mervekasikci/MLGeneSelection/main/Functions/biosign_gs.R")
...
result_biosigner <- foreach(i = 1:5, .combine=comb, .multicombine = TRUE, 
                            .errorhandling = "remove",
                            .init=list(list()),
                            .packages = c("caret", "biosigner", "assertthat")) %dopar% {
                              ...
                              set.seed(123)
                              model_biosign <- biosign_gs(t(x_train), y_train)
                              list(model_biosign[[2]])
                            } 

getDoParWorkers()                  
stopCluster(cl)

gene_list_biosigner <- unlist(result_biosigner[[1]])
gene_list_biosigner <- table(gene_list_biosigner)
gene_list_biosigner <- as.data.frame(gene_list_biosigner)
gene_list_biosigner <- subset(gene_list_biosigner, Freq>=2) 

write.csv(gene_list_biosigner, "gene_list_biosigner.csv")