...
source("https://raw.githubusercontent.com/mervekasikci/MLGeneSelection/main/Functions/GMDH_gs.R")
...
result_gmdh <- foreach(i = 1:5, .combine=comb, .multicombine = TRUE, 
                       .errorhandling = "remove",
                       .init=list(list()),
                       .packages = c("GMDH2", "caret", "assertthat")) %dopar% {
                         ...
                         set.seed(123)
                         model_GMDH<-GMDH_gs(t(x_train), as.factor(y_train), train.percent = 0.75)
                         list(model_GMDH[[2]])
                       } 
...
gene_list_gmdh <- unlist(result_gmdh[[1]])
gene_list_gmdh <- table(gene_list_gmdh)
gene_list_gmdh <- as.data.frame(gene_list_gmdh)
gene_list_gmdh <- subset(gene_list_gmdh, Freq>=2) 

write.csv(gene_list_gmdh, "gene_list_gmdh.csv")
